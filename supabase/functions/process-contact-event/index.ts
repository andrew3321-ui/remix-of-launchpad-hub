...
const validSources = ["activecampaign", "manychat", "uchat", "manual"] as const;
...
function scoreRecord(record: { primary_name?: string | null; primary_email?: string | null; primary_phone?: string | null; normalized_phone?: string | null; data?: unknown; }) {
...
function chooseValue(currentValue: string | null | undefined, incomingValue: string | null | undefined, preferIncoming: boolean) {
...
Deno.serve(async (request) => {
  ...
  if (launchError || !launch) {
    return jsonResponse({ error: "Launch not found" }, 404);
  }
  ...
  if (eventInsertError || !event) {
    return jsonResponse({ error: "Failed to create inbound event", details: eventInsertError?.message }, 500);
  }
  ...
  if (!normalizedEmail && !canonicalPhone) {
    ...
    return jsonResponse({ status: "rejected", reason: "missing_valid_email_or_phone" }, 422);
  }
  ...
  if (candidateIds.size > 0) {
    ...
    if (matchedContacts && matchedContacts.length > 0) {
      existingContact = [...matchedContacts].sort((left, right) => scoreRecord(right) - scoreRecord(left))[0] as Record<string, unknown>;
    }
  }
  ...
  if (existingContact && settings.auto_merge_duplicates) {
    const preferIncoming =
      ...
      incomingScore > scoreRecord(existingContact as { primary_name?: string | null; primary_email?: string | null; primary_phone?: string | null; normalized_phone?: string | null; data?: unknown; });

    const mergedData = {
      ...asRecord(existingContact.data),
      ...
      sources: uniqueValues([
        ...(Array.isArray(asRecord(existingContact.data).sources) ? (asRecord(existingContact.data).sources as string[]) : []),
        body.source,
      ]),
    };
    ...
    const { data: updatedContact, error: updateError } = await supabase
      ...
      .update({
        ...
        normalized_phone: chooseValue(existingContact.normalized_phone as string | null | undefined, canonicalPhone, preferIncoming),
    ...
    if (updateError || !updatedContact) {
      return jsonResponse({ error: "Failed to merge contact", details: updateError?.message }, 500);
    }
    ...
    if (createError || !createdContact) {
      return jsonResponse({ error: "Failed to create contact", details: createError?.message }, 500);
    }
